...
namespace detail
{
  ...
  struct RegularDecomposer
  {
                      ...
                      wrap(wrap_), ghosts(ghosts_), divisions(divisions_)
    {
      ...
      //ghosts_left = CoordinateVector();
      //ghosts_right = CoordinateVector();
      //ghosts_left.resize(dim);
      //ghosts_right.resize(dim);
    }

    //void            set_elastic_ghosts(const bool& elastic_ghosts_); // added by Jiang
    //void            set_ghost_block_size(const int* ghost_block_size_); // added by Jiang

    //void            compute_min_offset(const DivisionsVector coord, const Bounds bound, int* coords); // added by Jiang
    //void            compute_max_offset(const DivisionsVector coord, const Bounds bound, int* coords); // added by Jiang
    ...
    //bool              elastic_ghosts; // added by Jiang
    //int               ghost_block_size[4]; // added by Jiang
    //CoordinateVector  ghosts_left; // added by Jiang
    //CoordinateVector  ghosts_right; // added by Jiang
...
compute_max_offset(const DivisionsVector coord, const Bounds bound, int* coords)
{
  Bounds core = bound;
  for (int i = 0; i < dim; i ++) {
    while (true) {
      core.min[i] = detail::BoundsHelper<Bounds>::from(coords[i], divisions[i], domain.min[i], domain.max[i], share_face[i]);
      core.max[i] = detail::BoundsHelper<Bounds>::to  (coords[i], divisions[i], domain.min[i], domain.max[i], share_face[i]);
      if (bound.max[i] >= core.min[i] && bound.max[i] <= core.max[i]) break;
      coords[i] ++;
      if (coords[i] >= divisions[i]) {
        coords[i] = divisions[i]-1;
        break;
      }
    }
  }
...
void
diy::RegularDecomposer<Bounds>::
compute_min_offset(const DivisionsVector coord, const Bounds bound, int* coords)
{
  Bounds core = bound;
  for (int i = 0; i < dim; i ++) {
    while (true) {
      core.min[i] = detail::BoundsHelper<Bounds>::from(coords[i], divisions[i], domain.min[i], domain.max[i], share_face[i]);
      core.max[i] = detail::BoundsHelper<Bounds>::to  (coords[i], divisions[i], domain.min[i], domain.max[i], share_face[i]);
      if (bound.min[i] >= core.min[i] && bound.min[i] <= core.max[i]) break;
      coords[i] --;
      if (coords[i] < 0) {
        coords[i] = 0;
        break;
...
void
diy::RegularDecomposer<Bounds>::
set_elastic_ghosts(const bool& elastic_ghosts_)
{
  elastic_ghosts = elastic_ghosts_;
}
...
void
diy::RegularDecomposer<Bounds>::
set_ghost_block_size(const int* ghost_block_size_)
{
  for (int i = 0; i < dim; i ++)
    ghost_block_size[i] = ghost_block_size_[i];
}
*/
//! \ingroup Decomposition
     ...
     const 
{
  ...
  if (!add_ghosts)
    return;
#if 0
  if (elastic_ghosts) { // added by Jiang
    Bounds core; // here ghost is divided into left ghost and right ghost
    for (int i = 0; i < dim; i ++) {
      core.min[i] = bounds.min[i];
      core.max[i] = bounds.max[i];
    }

    float bounds_max[dim], bounds_min[dim];
    for (int i = 0; i < dim; i ++) {
      float add = (float)ghost_block_size[i]/2. - (float)(core.max[i]-core.min[i]+1)/2.;
      assert (add >= 0);
      bounds_max[i] = core.max[i] + add;
      bounds_min[i] = core.min[i] - add;

      if (ghost_block_size[i] > (domain.max[i] - domain.min[i] + 1)) {
        bounds_max[i] = domain.max[i];
        bounds_min[i] = domain.min[i];
      } else {
        if (bounds_max[i] > domain.max[i]) {
          bounds_max[i] = domain.max[i];
          bounds_min[i] = bounds_max[i] - ghost_block_size[i] + 1;
        }

        if (bounds_min[i] < domain.min[i]) {
          bounds_min[i] = domain.min[i];
          bounds_max[i] = bounds_min[i] + ghost_block_size[i] - 1;
        }
      }
    }

    for (int i = 0; i < dim; i ++) {
      bounds.max[i] = std::floor(bounds_max[i]);
      bounds.min[i] = std::floor(bounds_min[i]);
      ghosts_left[i] = bounds.min[i] - core.min[i];
      ghosts_right[i] = bounds.max[i] - core.max[i];
      assert (bounds.max[i] >= core.max[i] && bounds.min[i] <= core.min[i]);
    }
  } 
#endif
...
void
diy::RegularDecomposer<Bounds>::
fill_divisions(std::vector<int>& divisions) const // refined by Jiang
{
    ...
    for (int i = factors.size() - 1; i >= 0; --i)
    {
        ...
        Coordinate min =
            detail::BoundsHelper<Bounds>::from(0,
                                               missing_divs[n_dim].nb * factors[i],
                                               domain.min[missing_divs[n_dim].dim],
                                               domain.max[missing_divs[n_dim].dim],
                                               share_face[missing_divs[n_dim].dim]);
        Coordinate max =
            detail::BoundsHelper<Bounds>::to(0,
                                             missing_divs[n_dim].nb * factors[i],
                                             domain.min[missing_divs[n_dim].dim],
        ...
        {
            missing_divs[n_dim].nb    *= factors[i];
            missing_divs[n_dim].b_size = max - min;
        }
        else
        {
            std::ostringstream oss;
...
#if 0
    if (elastic_ghosts) {
      l = p[axis] - ghosts_left[axis];
      r = p[axis] + ghosts_right[axis];
    } else 
#endif
    l = p[axis] - ghosts[axis];
    r = p[axis] + ghosts[axis];
